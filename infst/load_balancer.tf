...
# Create a target group for port 8001 Prod (RDS)
resource "aws_lb_target_group" "prod_app_tg_8001" {
  name     = "app-tg-8001-prod"
  port     = 8001
  protocol = "HTTP"
...
# Create a target group for port 8002 Prod (Redis)
resource "aws_lb_target_group" "prod_app_tg_8002" {
  name     = "app-tg-8002-prod"
  port     = 8002
  protocol = "HTTP"
...
# Create a rule to forward to port 8001 target group for RDS
resource "aws_lb_listener_rule" "https_rule_rds" {
  listener_arn = aws_lb_listener.https.arn
  priority     = 100
...
# Create a rule to forward to port 8002 target group for Redis
resource "aws_lb_listener_rule" "https_rule_redis" {
  listener_arn = aws_lb_listener.https.arn
  priority     = 200
